...
const MyJobs = () => {
    ...
    // Set current loading page
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 4;

    useEffect(() => {
        setIsLoading(true);
        fetch(`http://localhost:5000/my/jobs/sadi@gmail.com`)
            .then(res => {
                if (!res.ok) {
            ...
            .then(data => {
                setJobs(data);
                setFilteredJobs(data); // Set the initial filtered jobs list
                setIsLoading(false);
            })
            .catch(err => {
    ...
    return (
        <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4'>
            <div className='my-jobs-container text-center'>
            ...
            <section className="py-1 bg-blueGray-50 flex justify-center">
                    ...
                    <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded">
                        ...
                        <div className="block w-full overflow-x-auto">
                            <table className="items-center bg-transparent w-full border-collapse">
                                ...
                                {isLoading ? (
                                    <div className='flex item-center justify-center h-20'><p>Loading.....</p></div>
                                ) : (
                                    <tbody>
                                        {currentJobs.map((job, index) => (
                                            <tr key={index}>
                                                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left text-blueGray-700 ">{index + 1 + indexOfFirstItem}</td>
